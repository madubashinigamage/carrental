...
<?php 
$id=intval($_GET['id']);
$sql ="SELECT tblvehicles.*,tbltype.BrandName,tbltype.id as bid from tblvehicles join tbltype on tbltype.id=tblvehicles.VehiclesBrand where tblvehicles.id=:id";
$query = $dbh -> prepare($sql);
$query-> bindParam(':id', $id, PDO::PARAM_STR);
$query->execute();
...
<?php $ret="select id,BrandName from tbltype";
